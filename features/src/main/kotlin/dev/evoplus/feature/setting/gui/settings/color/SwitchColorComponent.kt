...
class SwitchColorComponent(state: Boolean, initial: Color, private val allowAlpha: Boolean) : SettingComponent() {

    private var active = false

    private val block = UIBlock().constrain {
        width = 100.percent
        height = (13 + 10).pixels
    } childOf this

    private val switch = SwitchComponent(state) childOf block
    ...
    private val upArrow by ShadowIcon(SettingPalette.ARROW_UP_7X4, buttonShadow = true).constrain {
        x = 5.pixels(alignOpposite = true)
        y = 7.5.pixels
        width = 9.pixels
        height = 5.pixels
    ...
    init {
        ...
        enableEffect(OutlineEffect(SettingPalette.getComponentBorder(), 1f).bindColor(SettingPalette.componentBorder))
    ...
    fun callObserver() {
        changeValue(SwitchColor(switch.enabled.get(), colorPicker.getCurrentColor()))
    }
