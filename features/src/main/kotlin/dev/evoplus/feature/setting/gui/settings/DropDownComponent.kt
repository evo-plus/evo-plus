...
import dev.evoplus.feature.setting.utils.ReadOnlyState
...
internal class DropDownComponent(
    ...
    private val currentSelectionText by UIText(shadowColor = SettingPalette.getTextShadow()).bindText(selectedText).constrain {
        x = 5.pixels
        y = CenterConstraint()
        color = SettingPalette.getTextColor((selectAreaHovered and writableExpandedState)).toConstraint()
    } childOf selectedArea

    private val iconState = writableExpandedState.map {
        if (it) {
            SettingPalette.ARROW_UP_7X4
        } else {
            SettingPalette.ARROW_DOWN_7X4
        }
    }
    ...
    private val downArrow by ShadowIcon(iconState, BasicState(true)).constrain {
        ...
        y = CenterConstraint()
    }.rebindPrimaryColor(SettingPalette.getTextColor(selectAreaHovered)) childOf selectedArea


    private val expandedBlock by UIBlock(SettingPalette.buttonHighlight).constrain {
        y = SiblingConstraint()
        width = 100.percent
    ...
    // Height set in init
    private val expandedContentArea by UIBlock(SettingPalette.componentBackground).constrain {
        x = CenterConstraint()
        width = 100.percent
    ...
    private val scrollbar by UIBlock(SettingPalette.midGray).constrain {
        width = 100.percent
    ...
    init {
        ...
        setColor((selectAreaHovered or expandedState).map {
            if (it) {
                SettingPalette.getButtonHighlight()
            } else {
                SettingPalette.getComponentBackgroundHighlight()
            }
        }.toConstraint())
        ...
        options.withIndex().forEach { (index, value) ->
            ...
            optionContainer.setColor(hovered.map {
                if (it) {
                    SettingPalette.getButton()
                } else {
                    SettingPalette.getComponentBackground()
                }
            }.toConstraint())
            val text by UIText(value).constrain {
                x = 5.pixels
                y = CenterConstraint()
                color = SettingPalette.getTextColor(hovered).toConstraint()
