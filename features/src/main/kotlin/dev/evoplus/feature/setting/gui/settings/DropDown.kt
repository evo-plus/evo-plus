...
class DropDown(
    ...
    outlineEffect: OutlineEffect? = OutlineEffect(SettingPalette.getDivider(), 1f).bindColor(SettingPalette.dividerState),
    ...
    private val currentSelectionText by UIText(options[selected]).constrain {
        x = 5.pixels()
        y = 6.pixels()
        color = SettingPalette.midTextState.toConstraint()
    ...
    init {
        constrain {
            ...
            height = 20.pixels()
            color = SettingPalette.darkHighlightState.toConstraint()
        }
    ...
    private fun expand() {
        ...
        mappedOptions.forEach {
            it.setColor(SettingPalette.midTextState.toConstraint())
        }
    ...
    private fun hoverText(text: UIComponent) {
        text.animate {
            setColorAnimation(Animations.OUT_EXP, 0.25f, SettingPalette.brightTextState.toConstraint())
        }
    }

    private fun unHoverText(text: UIComponent) {
        text.animate {
            setColorAnimation(Animations.OUT_EXP, 0.25f, SettingPalette.midTextState.toConstraint())
        }
    }
